...
class Node {
    ...
    int distance(Node n1, Node n2) {
        ...
        return (int) (41701090.90909090909090909091 * Math.asin(Math.sqrt(
                sin_bredde * sin_bredde + n1.cos_latitude * n2.cos_latitude * sin_lengde * sin_lengde)));
    }
...
class Edge {
    ...
    private double time; //weight
    private Node from;
    private Node to;

    public Edge(Node from, Node to, double time) {
        this.from = from;
        this.to = to;
        this.time = time;
    }
...
class DikjstraAlgorithm {
    public static void main(String[] args) throws Exception{
        ...
        File file = new File("C:\\Users\\Ådne\\Google Drive\\NTNU\\3. semester\\Algoritmer og datastrukturer\\Øvinger-algdat\\src\\Oving8\\" + fileName + ".txt");
    ...
    public static class Graph {
        ...
        public void findShortestPaths(int sourceName){
            ...
            for(int i = 0; i < maxSize; i++){
                ...
                if(vertices[i].time == Integer.MAX_VALUE) {
                    cost = "unåelig";
                    last = "";
                } else {
                    cost = vertices[i].time + "";
                }
        ...
        public void applyDikjstraAlgorith(Vertex src) {
            ...
            while (!heap.isEmpty()) {
                ...
                while (temp != null) {
                    ...
                    if (vertices[temp.index].time > u.time + temp.weight) {
                        vertices[temp.index].time = u.time + temp.weight;
